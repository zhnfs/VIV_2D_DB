function plotDatabaseOri(ILCFHydroModelobj,Y_vec, X_vec, theta_vec, vr_vec,theta_set_disp, y_lim_Cmy,y_lim_Cmx, y_lim_CLv,y_lim_CDv,x_lim, ...
...
for i = 1:length(X_vec)
    for j = 1:length(Y_vec)
        ...
        for k = 1:length(theta_vec)
            h(k) = plot1D_f_ori(ILCFHydroModelobj , 'CLv', Y_vec(j), X_vec(i), theta_vec(k), vr_vec);
            set(h(k),'Color',col(k,:))
            set(h(k),'Line','-')
        ...
        saveas(gcf,[outputDir filesep 'CLv' 'X' num2str(X_vec(i)) 'Y' num2str(Y_vec(j)) marker 'Ori.jpg'])
        saveas(gcf,[outputDir filesep 'CLv' 'X' num2str(X_vec(i)) 'Y' num2str(Y_vec(j)) marker 'Ori.fig'])
        close
    end
end
% plot all CDv
for i = 1:length(X_vec)
    for j = 1:length(Y_vec)
        figure
        for k = 1:length(theta_vec)
            h(k) = plot1D_f_ori(ILCFHydroModelobj , 'CDv', Y_vec(j), X_vec(i), theta_vec(k), vr_vec);
            set(h(k),'Color',col(k,:))
            set(h(k),'Line','-')
            set(h(k),'Marker',shp{k})
            ylim(y_lim_CDv)
        ...
        uistack(legend1, 'top');
        %         set(legend1,'Position',[10 20 100 200]);
        saveas(gcf,[outputDir filesep 'CDv' 'X' num2str(X_vec(i)) 'Y' num2str(Y_vec(j)) marker 'Ori.jpg'])
        saveas(gcf,[outputDir filesep 'CDv' 'X' num2str(X_vec(i)) 'Y' num2str(Y_vec(j)) marker 'Ori.fig'])
        close
    end
end
% plot all Cmy
...
for i = 1:length(X_vec)
    for j = 1:length(Y_vec)
        figure
        for k = 1:length(theta_vec)
            h(k) = plot1D_f_ori(ILCFHydroModelobj , 'Cmy', Y_vec(j), X_vec(i), theta_vec(k), vr_vec);
            set(h(k),'Color',col(k,:))
            set(h(k),'Line','-')
            set(h(k),'Marker',shp{k})
            ylim(y_lim_Cmy)
        ...
        uistack(legend1, 'top');
        %         set(legend1,'Position',[10 20 100 200]);
        saveas(gcf,[outputDir filesep 'Cmy' 'X' num2str(X_vec(i)) 'Y' num2str(Y_vec(j)) marker 'Ori.jpg'])
        saveas(gcf,[outputDir filesep 'Cmy' 'X' num2str(X_vec(i)) 'Y' num2str(Y_vec(j)) marker 'Ori.fig'])
        close
    end
end
...
for i = 1:length(X_vec)
    for j = 1:length(Y_vec)
        figure
        for k = 1:length(theta_vec)
            h(k) = plot1D_f_ori(ILCFHydroModelobj , 'Cmx', Y_vec(j), X_vec(i), theta_vec(k), vr_vec);
            set(h(k),'Color',col(k,:))
            set(h(k),'Line','-')
            set(h(k),'Marker',shp{k})
            ylim(y_lim_Cmx)
        ...
        uistack(legend1, 'top');
        %         set(legend1,'Position',[10 20 100 200]);
        saveas(gcf,[outputDir filesep 'Cmx' 'X' num2str(X_vec(i)) 'Y' num2str(Y_vec(j)) marker 'Ori.jpg'])
        saveas(gcf,[outputDir filesep 'Cmx' 'X' num2str(X_vec(i)) 'Y' num2str(Y_vec(j)) marker 'Ori.fig'])
        close
    end
end
