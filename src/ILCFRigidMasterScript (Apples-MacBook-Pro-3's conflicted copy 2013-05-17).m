...
outputDir =[ILCFVIVRoot filesep 'output' filesep 'ICFLMotionZDMiT_RobustSmooth_newrun5'];
...
%             x0(3) = 360.*(rand(1)-0.5);
    ...
    %         
        ...
        %     x0(5) = Vrn_x(freqRatioInd,i);
            ...
            f = @(x) myILCFfun(x, RigCylModel2Dobj(freqRatioInd,i), ILCFHydroModelobj, Vrn_y(freqRatioInd,i), Vrn_x(freqRatioInd,i));
...
%         result_uniq(:,3) = result_uniq(:,3).*360;     
       ...
       j=1;
       f2 = @(x) myILCFfun2(x, RigCylModel2Dobj(freqRatioInd,i), ILCFHydroModelobj, Vrn_y(freqRatioInd,i), Vrn_x(freqRatioInd,i));
...
%        remove solutions has Amp Y <0.15  for HZ  0.25 for JMD,
       ...
       result_uniq(result_uniq(:,2)<0.05,:) =[];
...
save ICFLMotionZDMiT_RobustSmooth_run100.mat
