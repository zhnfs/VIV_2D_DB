% function ILCFRigidMasterScript_ZDMiT_RobustSmooth_FixedTheta45Yp75Xp15()
...
outputDir =[ILCFVIVRoot filesep 'output' filesep 'ICFLMotionZDMiT_RobustSmooth_FixedTheta45Yp75Xp15_run10'];
...
ILCFHydroModelobj = ILCFHydroModelZDMiT_Sim();
...
%     
%     % begin added for mass ratio 1
%     FluidVel = FluidVel .* sqrt((mRatio+1)/(1+1));
%     % end added for mass ratio 1
...
%              lb =[0.25 0 -1 4.5 2 -RigCylModel2Dobj.MassRatio_y -RigCylModel2Dobj.MassRatio_x 0 0 0 0];
%              ub =[1.5 0.75 1 20 20 5 5 5 5 0.1 0.1];
             lb =[0 0 -1 0 0 -RigCylModel2Dobj.MassRatio_y -RigCylModel2Dobj.MassRatio_x 0 0 0 0];
...
%         result_uniq(:,3) = result_uniq(:,3).*360;     
       ...
       j=1;
       f2 = @(x) myILCFfun2(x, RigCylModel2Dobj, ILCFHydroModelobj, Vrn_y(freqRatioInd,i), Vrn_x(freqRatioInd,i));
       while j <=size(result_uniq,1)
            f0 = feval(f2, result_uniq(j,:));
                          
            if max(abs(f0(1:5)))>0.01
                result_uniq(j,:) = [];
            else
                j = j+1;
            end
...
compIndx = [9:25];
   ...
   close
%    % minimum residuals
%     plot_min_res(vrn_fr, vr_fr, Yad_fr, Xad_fr, Vrn, vrn_pred, x, res, numRun, freqRatio, freqRatioInd, compIndx, outputDir)
...
%  
    ...
    save ICFLMotionZDMiT_RobustSmooth_FixedTheta45Yp75Xp15run10.mat
